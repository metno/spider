# spider estimate vertical profile
#------------------------------------------------------------------------------
spider_estvertprof <- function( box_o_nearest_halfwidth = 100000,  # m
...
#------------------------------------------------------------------------------
  ...
  #
  if ( !is.na( argv$cores)) {
    res_step1 <- t( mcmapply( spider_estvertprof_step1,
                     1:nag_tar,
                     mc.cores = argv$cores,
                     SIMPLIFY = T,
                     MoreArgs = list(
                       box_o_nearest_halfwidth = argv$evp_halfbox,
                       pmax    = argv$evp_pmax,
  ...
  # no-multicores
    ...
    res_step1 <- t( mapply(   spider_estvertprof_step1,
                     1:nag_tar,
                     SIMPLIFY = T,
                     MoreArgs = list(
                       box_o_nearest_halfwidth = argv$evp_halfbox,
  ...
  cat( "EVP - step2, start ...")
  if ( !is.na( argv$cores)) {
    res <- mcmapply( spider_estvertprof_step2,
                     1:nma_tar,
                     mc.cores = argv$cores,
                     SIMPLIFY = T,
                     MoreArgs = list(
  ...
  # no-multicores
     ...
     res <- mapply( spider_estvertprof_step2,
                     1:nma_tar,
                     SIMPLIFY = T,
                     MoreArgs = list(
                       xma_tar    = xma_tar,
...
#     res,par,ixok,ixno, resl)
  return( list( res_out=res, ix=ix_ma))
}
