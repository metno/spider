...
# 
#------------------------------------------------------------------------------
spider_estvertprof_step1 <- function( i,
...
#  if (i%%1000==0) print(paste(i,Sys.time()-t0))
  ...
  if ( !any( flagx <- ( deltax < box_o_nearest_halfwidth))) return( rep( NA, 2)) 
  deltay <- abs( yag_tar[i]-yor_tar[flagx])
  if ( !any( flagy <- ( deltay < box_o_nearest_halfwidth))) return( rep( NA, 2)) 
  ixa <- which( flagx)[flagy]
  if ( length( ixa) == 0) return( rep( NA, 2)) 
  #
  if ( length(ixa) > pmax) {
  ...
  # ui %*% theta - ci >= 0
  opt <- constrOptim( theta = c( lopt$minimum, gamma),
                      grad  = NULL,
                      f     = opt_vertprof_basic_2pars,
